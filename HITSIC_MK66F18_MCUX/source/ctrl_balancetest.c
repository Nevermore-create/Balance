...
void MENU_DataSetUpTEST(void)
{
    ...
    MENU_ListInsert(menu_menuRoot, MENU_ItemConstruct(menuType, control, "Control", 0, 0));
    {
        ...
        MENU_ListInsert(control, MENU_ItemConstruct(varfType, &filter_angle, "filter_angle", 11, menuItem_data_global));
        MENU_ListInsert(control, MENU_ItemConstruct(varfType, &ctrl_angSet, "ctrl_angSet", 11, menuItem_data_global));
    }
    //TODO: 在这里添加子菜单和菜单项
}
...
void CTRL_AngCtrl(void *userData)
{
    ...
    if(1 == ctrl_angCtrlEn[0])
    {
        ...
        ctrl_angPidOutput = ctrl_angPidOutput < 90.0f ? ctrl_angPidOutput : 90.0f;  ///< 限幅
    }
    else
    {
